...
# noinspection SpellCheckingInspection
hiring_client_headers = ['contractor_name']
...
BASE_GENERIC_COMPANY_NAME_WORDS = ['construction', 'contracting', 'industriel', 'industriels', 'service',
                                   ...
                                   'technologies', 'company', 'corporation', 'installations', 'enr', 'electrical']
...
def add_analysis_data(hc_row, cbx_row, ratio_company=None, ratio_address=None, contact_match=None):
    ...
    print('   --> ', cbx_company, cbx_row[CBX_ID], ratio_company, ratio_address, contact_match)
    ...
    is_qualified = False
    # sub_price_cad = float(cbx_row[CBX_SUB_PRICE_CAD])
    # employee_price_cad = float(cbx_row[CBX_EMPL_PRICE_CAD])
    hiring_client_contractor_summary = f'{hc_row[HC_COMPANY]}'
...
if __name__ == '__main__':
    ...
    for index, hc_row in enumerate(hc_data):
        ...
        for cbx_row in cbx_data:
            ...
            ratio_company_fr = fuzz.token_sort_ratio(clean_hc_company, cbx_company_fr)
            ratio_company_en = fuzz.token_sort_ratio(clean_hc_company, cbx_company_en)
            ...
            if ((ratio_company >= args.ratio_company)):
                matches.append(
                    add_analysis_data(hc_row, cbx_row, ratio_company))
        ids = []
        best_match = 0
        matches = sorted(matches, key=lambda x: (x['hiring_client_count'], x["ratio_company"]),
                         reverse=True)
        ...
        # create_in_cognibox = False if len(uniques_cbx_id) else True
    ...
    dims = {}
    # for sheet in sheets:
    tab = Table(displayName=(out_ws.title or 'all'),
    ...
    out_ws.column_dimensions[get_column_letter(HC_HEADER_LENGTH+analysis_headers.index("hc_contractor_summary")+1)].width = 150
    out_ws.column_dimensions[get_column_letter(HC_HEADER_LENGTH+analysis_headers.index("analysis")+1)].width = 150
    ...
    for i in range(2, len(hc_data)+1):
        out_ws.cell(i, HC_HEADER_LENGTH+analysis_headers.index("analysis")+1).alignment = Alignment(wrapText=True)
        out_ws.cell(i, HC_HEADER_LENGTH+analysis_headers.index("hc_contractor_summary")+1).alignment = Alignment(wrapText=True)
